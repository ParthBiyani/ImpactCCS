// ignore_for_file: avoid_print
import 'package:app/services/bill.dart';
import 'package:gsheets/gsheets.dart';

class BillSheetsApi {
  static const _credentials = r'''
{
  "type": "service_account",
  "project_id": "upbusiness-425717",
  "private_key_id": "23f8e7c8263859b2acb7b5ef521073fb2c071b2a",
  "private_key": "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n",
  "client_email": "upbusiness@upbusiness-425717.iam.gserviceaccount.com",
  "client_id": "111905074481569731196",
  "auth_uri": "https://accounts.google.com/o/oauth2/auth",
  "token_uri": "https://oauth2.googleapis.com/token",
  "auth_provider_x509_cert_url": "https://www.googleapis.com/oauth2/v1/certs",
  "client_x509_cert_url": "https://www.googleapis.com/robot/v1/metadata/x509/upbusiness%40upbusiness-425717.iam.gserviceaccount.com",
  "universe_domain": "googleapis.com"
}

''';

  static const _spreadsheetId = '1thi11zOH-htZ1C1yxdUzkeXMN5lKmutTYTfwvo7ynjI';

  static final _gsheets = GSheets(_credentials);
  static Worksheet? _billSheet;

  static Future init() async {
    try {
      final spreadsheet = await _gsheets.spreadsheet(_spreadsheetId);
      _billSheet = await _getWorkSheet(spreadsheet, title: 'Bills');

      final firstRow = BillFields.getFields();
      _billSheet!.values.insertRow(1, firstRow);
    } catch (e) {
      print('Init Error: $e');
    }
  }

  static Future<Worksheet> _getWorkSheet(
    Spreadsheet spreadsheet, {
    required String title,
  }) async {
    try {
      return await spreadsheet.addWorksheet(title);
    } catch (e) {
      return spreadsheet.worksheetByTitle(title)!;
    }
  }

  static Future<List<Bill>> getAll() async {
    print("Entered the getAll method");
    if (_billSheet == null) {
      print("Bill Sheet is null");
      return <Bill>[];
    }

    final bill = await _billSheet!.values.map.allRows();
    // return bill == null ? <Bill>[] : bill.map(Bill.fromJson).toList();
    if (bill == null) {
      print("Bill is null");
      return <Bill>[];
    } else
      return bill.map(Bill.fromJson).toList();
  }

  static Future<Bill?> getByType(String type) async {
    if (_billSheet == null) return null;

    final json = await _billSheet!.values.map.rowByKey(type, fromColumn: 1);
    return json == null ? null : Bill.fromJson(json);
  }

  static Future insert(List<Map<String, dynamic>> rowList) async {
    print("Entered insert method");
    if (_billSheet == null) {
      print("Bill sheet is null");
      return;
    }

    await _billSheet!.values.map.appendRows(rowList);
  }
}
